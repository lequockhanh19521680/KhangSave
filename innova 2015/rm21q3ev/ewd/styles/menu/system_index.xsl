...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="TitleList">
		<table width='80%' border='1' cellspacing='0' cellpadding='0' bordercolor='#FFFFFF'>
			<tr class='gray2'><td>System Circuit</td></tr>
	...
	<xsl:template name="Title">
		...
		<xsl:param name="pdf">No pdf</xsl:param>
		<xsl:param name="name">No Name</xsl:param>
		<div class='titlelist' onMouseOver='call_runOnMouseOver()' onMouseOut='call_runOnMouseOut()'>
			<xsl:attribute name="onClick">call_runOnClickTitle('<xsl:value-of select="$code"/>', '<xsl:value-of select="$type"/>', '<xsl:value-of select="$pdf"/>')</xsl:attribute>
